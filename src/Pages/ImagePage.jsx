...
const ImagePage = () => {
  ...
  return (<>
    ...
    <Carousel>
     <Carousel.Item>
      <CardGroup className='imgcardgroup'>
      {relateddata.slice(0,5).map((x,y)=><Card key={y}><img src={x.webformatURL} onClick={()=>setImage(x.webformatURL)}/></Card>)}
      </CardGroup>
      </Carousel.Item>

      <Carousel.Item>
      <CardGroup className='imgcardgroup'>
      {relateddata.slice(5,10).map((x,y)=><Card key={y}><img src={x.webformatURL} onClick={()=>setImage(x.webformatURL)} /></Card>)}
      </CardGroup>
      </Carousel.Item>

      <Carousel.Item>
      <CardGroup className='imgcardgroup'>
      {relateddata.slice(10,15).map((x,y)=><Card key={y}><img src={x.webformatURL} onClick={()=>setImage(x.webformatURL)} /></Card>)}
      </CardGroup>
      </Carousel.Item>

      <Carousel.Item>
      <CardGroup className='imgcardgroup'>
      {relateddata.slice(15,20).map((x,y)=><Card key={y}><img src={x.webformatURL} onClick={()=>setImage(x.webformatURL)} /></Card>)}
      </CardGroup>
      </Carousel.Item>

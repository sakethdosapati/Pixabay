...
export default function Pixabay(){
    ...
    useEffect(()=>{
        fetch(`https://pixabay.com/api/?key=46193785-c6d0ac0c64a1e4dec45c8d153&q=${search}&image_type=photo&pretty=true`).then(x=>x.json()).then(x=>{
           setdata(x.hits)
    ...
    return(
        ...
        <div id="div1">
            ...
            <input id="input" type="text" onChange={(e)=>setSearch(e.target.value)} placeholder="search"/>
           
        </div>
        <div id="images">
            
        {data.map(x=><img onClick={()=>navigate("/image",{state:{x,search,data}})}id="img"src={x.webformatURL} />)}
